...
function Dashboard({ setIsAuthenticated }) {
  ...
  const fetchTasks = async () => {
    try {
      ...
      const response = await axios.get(`${process.env.REACT_APP_API_URL || "http://localhost:3000"}/api/tasks`, {
  ...
  const handleDeleteTask = async (taskId) => {
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`${process.env.REACT_APP_API_URL || "http://localhost:3000"}/api/tasks/${taskId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
